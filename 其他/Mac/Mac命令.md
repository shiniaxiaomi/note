...
通过修改bash_profile文件来将shell命令起别名，这样可以大大增加我们的工作效率

有以下几种方式：

- 我们可以修改用户目录下的`~/.bash_profile`文件，如果不存在，则自己创建

  只文件只针对该用户生效

- 我们也可以修改`/etc`目录下的`/etc/.bash_profile`文件，如果不存在，则自己创建
...
> `alias`是别名的关键字，`ll`是我们自己起的别名，`""`中间的内容是别名对应的内容

在修改完`.bash_profile`文件之后，我们需要执行`source .bash_profile`命令来使用文件生效
